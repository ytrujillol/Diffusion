function [A,b]=fNmatrix(M,v)



nvel=length(v); % velocity degrees of freedom==number of vertices.
...
%   waitbar(i/(2*mesh_f.ne),h)
    ...
    lA=flocalAedge(M(i,:),v); % compute local part of A
    A(lcol,lcol)=A(lcol,lcol)+lA;
    %%%%%%%%%%%%%%%%%%%% LOAD VECTOR         %%%%%%%%%%%%%%%%%%%%%%%%%%
    lb=flocalbprueba(M(i,:),v);
